...
export default class Tour {
  ...
  public startTour() {
    ...
    setTimeout(() => {
      if (
        !this.urlParamExists() ||
        document.querySelector(".shepherd-content")
      ) {
        return;
      }
  ...
  private addSteps() {
    ...
    this.tour.addStep({
      ...
      buttons: [
        {
          action() {
            if (!context.isMobile) context.copyDropDown();
            context.appendDropDown();
            return this.next();
          },
          text:
            context.english === null
    ...
    this.tour.addStep({
      ...
      popperOptions: {
        modifiers: [
          { name: "offset", options: { offset: [0, this.isMobile ? 20 : 60] } },
        ],
      },
    });
  ...
  private appendDropDown() {
    setTimeout(() => {
      const parentDiv: HTMLElement = document.querySelector(
        "div[class^='dropdownItemsWrapper']"
      );
      const childElements: any = parentDiv.querySelectorAll("*");
      childElements.forEach((childElement) => {
        console.log(childElement); // For example, print each child element
        childElement.tabIndex = -1;
        childElement.setAttribute("aria-hidden", "true");
        if (childElement.classList) {
          if (childElement.classList.contains("ms-FocusZone")) {
            childElement.classList.remove("ms-FocusZone");
  ...
  private cleanseUrl() {
    if (this.urlParamExists()) {
      let newUrl: string = window.location.href
        ...
        .replace("gcxLangTour", "");
      const newState: any = {
        additionalInformation: "Updated the URL after the tour.",
      };
